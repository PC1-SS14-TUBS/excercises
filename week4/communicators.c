...
int main(int argc, char **argv) {
	...
	//Print Comparison Id's for world, first and second group
	MPI_Barrier(MPI_COMM_WORLD);

	if (hasFirstGroup){
		printf("Proc name = %s | World rank = %d | First rank = %d\n", name, whoAmI_WorldComm, whoAmI_FirstComm);
	}

	MPI_Barrier(MPI_COMM_WORLD);

	if (hasSecondGroup){
		printf("Proc name = %s | World rank = %d | Second rank = %d\n", name, whoAmI_WorldComm, whoAmI_SecondComm);
	}
