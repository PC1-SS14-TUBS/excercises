...
#  endif
   ...
   thread_handles = malloc(thread_count*sizeof(pthread_t));  //Allocate memory for an array of threads
...
#  endif
   ...
   start_time = get_time();
   for (thread = 0; thread < thread_count; thread++)  //Iterate through the threads and start (Create) them
      pthread_create(&thread_handles[thread], NULL,   //Note that a pointer to the function Pth_mat_vect is used, so that 
         Pth_mat_vect, (void*) thread);               //such function will start upon thread creation. 

   for (thread = 0; thread < thread_count; thread++)  //Implementation of join sinchronization method. Remaining sections in the
      pthread_join(thread_handles[thread], NULL);     //program will be delayed until all the threads have finalized.
